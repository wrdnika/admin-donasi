...
<div class="max-w-2xl mx-auto p-6 bg-white rounded-2xl shadow-md space-y-6">
    ...
    <form action="{{ route('donation-reports.store') }}" method="POST" enctype="multipart/form-data" class="space-y-5">
        ...
        <div>
            ...
            <input type="file" name="report_image" id="report_image" accept="image/*" class="block w-full text-sm text-gray-900 border border-gray-300 rounded-lg cursor-pointer bg-gray-50 focus:outline-none focus:ring-green-500 focus:border-green-500 p-2.5" required>
